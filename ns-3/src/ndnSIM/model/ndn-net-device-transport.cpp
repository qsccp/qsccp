...
NetDeviceTransport::doSend(const Block& packet, const nfd::EndpointId& endpoint)
{
  ...
  // std::cout << "NetDeviceTransport: nfdPacket: " << packet.size() << ", ns3PacketSize: " << ns3Packet->GetSize() << std::endl;
  // send the NS3 packet
  m_netDevice->Send(ns3Packet, m_netDevice->GetBroadcast(),
                    L3Protocol::ETHERNET_FRAME_TYPE);
  // std::cout << ns3Packet->GetSerializedSize() << ", size: " << ns3Packet->GetSize() << std::endl;
  // NS_LOG_FUNCTION(this << ns3Packet->GetSerializedSize() << ", size: " << ns3Packet->GetSize());
}
