...
Producer::OnInterest(shared_ptr<const Interest> interest)
{
  ...
  // add by qjm, echo Interest's ToS
  int serviceClass = 7;
  if (interest->getServiceClass()) {
      serviceClass = *interest->getServiceClass();
      data->setServiceClass(serviceClass);
    NS_LOG_DEBUG("Interest have serviceClass: " << serviceClass);
  } else {
    NS_LOG_DEBUG("Interest have no serviceClass");
  }
