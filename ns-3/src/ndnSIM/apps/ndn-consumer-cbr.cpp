  ...
  static TypeId tid =
    TypeId("ns3::ndn::ConsumerCbr")
      ...
      .AddAttribute("Tos",
                    "Type of service",
                    UintegerValue(1),
                    MakeUintegerAccessor(&ConsumerCbr::tos),
                    MakeUintegerChecker<uint32_t>())
      .AddAttribute("Dsz",
                    "Data size",
                    UintegerValue(8800),
                    MakeUintegerAccessor(&ConsumerCbr::dsz),
                    MakeUintegerChecker<uint32_t>())
...
ConsumerCbr::ScheduleNextPacket()
{
  ...
  if (m_firstTime) {
    m_sendEvent = Simulator::Schedule(Seconds(0.0), &ConsumerCbr::SendPacket, this);
    m_firstTime = false;
  }
  else if (!m_sendEvent.IsRunning())
    m_sendEvent = Simulator::Schedule((m_random == 0) ? Seconds(1.0 / m_frequency)
                                                      : Seconds(m_random->GetValue()),
                                      &ConsumerCbr::SendPacket, this);
}
...
ConsumerCbr::SendPacket()
{
  ...
  interest->setTag(make_shared<lp::DSZTag>(this->dsz));
  interest->setTag(make_shared<lp::TosTag>(this->tos));
