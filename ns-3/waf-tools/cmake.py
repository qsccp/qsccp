...
def options(opt):
    ...
    '''
    opt.add_option('--cmake', dest='cmake', default=False, action='store_true',
                   help='select cmake for export/import actions')
...
class CMake(object):
    ...
    def get_tgen_content(self, tgen):
        ...
        _use = getattr(tgen, 'use', [])
        if not isinstance(_use, list):
            _use = _use.split(" ")
        _lib = getattr(tgen, 'lib', [])
        if not isinstance(_lib, list):
            _lib = _lib.split(" ")
        libs = _use + _lib
        if len(libs):
            content += '\n'
